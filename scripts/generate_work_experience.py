#!/usr/bin/env python3
"""
Script to parse resume.tex and generate work_experience.json
"""
...
def parse_resume_tex(file_path: str) -> List[Dict]:
    """Parse the LaTeX resume file and extract work experience."""
    ...
    for match in matches:
        job_title, date_range, company_name, location = match
        start_date, end_date = parse_date_range(date_range)
        
        # Find the description for this job
        # Look for the resumeItemListStart after this resumeSubheading
        # More flexible pattern to handle whitespace variations
        job_pattern = (
            r'\\resumeSubheading\s*\{' + re.escape(job_title) + r'\}\{' + re.escape(date_range) + r'\}\s*\{' +
            re.escape(company_name) + r'\}\{' + re.escape(location) + r'\}.*?\\resumeItemListStart(.*?)\\resumeItemListEnd'
        )
        job_section_match = re.search(job_pattern, work_section, re.DOTALL)
        
        description_items = []
        if job_section_match:
            items_section = job_section_match.group(1)
...
def main():
    ...
    json_path = 'data/work_experience.json'
    ...
    print(f"Generated data/work_experience.json with {len(experiences)} experiences")
